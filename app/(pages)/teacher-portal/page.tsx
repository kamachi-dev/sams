        ...
        try {
            ...
            if (format === 'pdf') {
                ...
                const drawFooter = (pageNum: number, totalPages: number) => {
                    ...
                    doc.text(`SF2 Report — ${sf2.courseName} (Section ${sf2.section}) — ${sf2.month} ${sf2.year}`, mL, pageH - 6);
                ...
                // ════════════════════════════════════════════════════
                // ── PAGE 1: SF2 Daily Attendance Report ──
                // ════════════════════════════════════════════════════
                ...
                doc.text('School Form 2 (SF2) Daily Attendance Report of Learners', pageW / 2, 11, { align: 'center' });
                curY = 24;
                ...
                const leftPairs: [string, string][] = [
                    ...
                    ['Course:', sf2.courseName],
                    ['Grade & Section:', `Section ${sf2.section}`],
                ];
                const rightPairs: [string, string][] = [
                    ['Teacher:', sf2.teacherName],
                    ['Report Month:', `${sf2.month} ${sf2.year}`],
                    ['Total Enrolled:', String(sf2.totalEnrolled)],
                ];
                ...
                leftPairs.forEach(([label, value], idx) => {
                    ...
                    doc.text(value, mL + 30, curY + idx * 5);
                });
                ...
                // ── SF2 Attendance Table (full-width) ──
                const dayOfWeekNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

                const sf2Head = [
                    [
                        { content: 'No.', rowSpan: 2, styles: { halign: 'center' as const, valign: 'middle' as const } },
                        { content: "LEARNER'S NAME", rowSpan: 2, styles: { halign: 'left' as const, valign: 'middle' as const } },
                ...
                const sf2Body = students.map((student: any, i: number) => {
                    const totalRecorded = student.actualRecords ?? (student.totalPresent + student.totalLate + student.totalAbsent);
                    const pct = totalRecorded > 0 ? ((student.totalPresent / totalRecorded) * 100).toFixed(1) + '%' : 'N/A';
                    const nd = student.totalNoDetection ?? 0;
                    return [
                        { content: String(i + 1), styles: { halign: 'center' as const, textColor: gray } },
                        student.name,
                        ...allDays.map(d => {
                            const status = student.dailyStatus[String(d)];
                        ...
                        { content: String(student.totalPresent), styles: { halign: 'center' as const, textColor: green, fontStyle: 'bold' as const } },
                        { content: String(student.totalLate), styles: { halign: 'center' as const, textColor: yellow, fontStyle: 'bold' as const } },
                        { content: String(student.totalAbsent), styles: { halign: 'center' as const, textColor: red, fontStyle: 'bold' as const } },
                ...
                for (const day of allDays) {
                    ...
                    for (const s of students) {
                        const st = s.dailyStatus[String(day)];
                        if (st === 'P') dP++;
                        else if (st === 'L') dL++;
                        else if (st === 'A') dA++;
                        else if (st === 'ND') dND++;
                ...
                doc.setFont('helvetica', 'bold');
                doc.setTextColor(...navy);
                doc.text(`Total Enrolled: ${sf2.totalEnrolled}     |     School Days: ${sf2.totalSchoolDays}     |     Legend:  P = Present    L = Late    A = Absent    - = No Detection`, mL + 4, curY + 6);
                ...
                doc.setFont('helvetica', 'bold');
                doc.setTextColor(255, 255, 255);
                doc.text('SAMS Analytics — Additional Insights', pageW / 2, 9, { align: 'center' });
                curY = 18;
                ...
                for (const s of students) {
                    ...
                    actualGL += s.totalLate;
                    actualGA += s.totalAbsent ?? 0;
                    actualGR += s.actualRecords ?? (s.totalPresent + s.totalLate + (s.totalAbsent ?? 0));
                }
                ...
                const perStudentBody = students.map((s: any, i: number) => {
                    const totalRec = s.actualRecords ?? (s.totalPresent + s.totalLate + (s.totalAbsent ?? 0));
                    const rate = totalRec > 0 ? ((s.totalPresent / totalRec) * 100).toFixed(1) + '%' : 'N/A';
                    const nd = s.totalNoDetection ?? 0;
                    const rc: [number, number, number] = totalRec > 0 ? (s.totalPresent / totalRec >= 0.8 ? green : s.totalPresent / totalRec >= 0.5 ? yellow : red) : gray;
                    return [
                        { content: String(i + 1), styles: { halign: 'center' as const, textColor: gray } },
                        s.name,
                        { content: String(s.totalPresent), styles: { halign: 'center' as const, textColor: green, fontStyle: 'bold' as const } },
                        { content: String(s.totalLate), styles: { halign: 'center' as const, textColor: yellow, fontStyle: 'bold' as const } },
                        { content: String(s.totalAbsent ?? 0), styles: { halign: 'center' as const, textColor: red, fontStyle: 'bold' as const } },
                        { content: String(nd), styles: { halign: 'center' as const, textColor: gray } },
                        { content: String(totalRec), styles: { halign: 'center' as const, fontStyle: 'bold' as const } },
                        { content: rate, styles: { halign: 'center' as const, fontStyle: 'bold' as const, textColor: rc } },
                    ];
                });
                ...
                autoTable(doc, {
                    startY: curY,
                    head: perStudentHead,
                    body: perStudentBody as any,
                    theme: 'grid',
                    styles: { fontSize: 8, cellPadding: 2, lineColor: [180, 180, 180], lineWidth: 0.15 },
                    headStyles: { fillColor: navy, textColor: [255, 255, 255], fontStyle: 'bold', halign: 'center', cellPadding: 2.5 },
                    columnStyles: {
                ...
                const sigRightX = pageW - mR - sigBlockW - 30;
                doc.setFontSize(9);
                doc.setFont('helvetica', 'bold');
                doc.setTextColor(...darkText);
                doc.text('Prepared by:', sigLeftX, curY);
                doc.text('Noted by:', sigRightX, curY);
                curY += 10;

                doc.setFontSize(11);
                doc.setTextColor(...navy);
                doc.text(sf2.teacherName, sigLeftX, curY);

                doc.setDrawColor(100, 100, 100);
                doc.line(sigRightX, curY + 1, sigRightX + sigBlockW, curY + 1);
                curY += 4;

                doc.setFontSize(7.5);
                doc.setFont('helvetica', 'normal');
                doc.setTextColor(...gray);
                doc.text('Class Adviser / Subject Teacher', sigLeftX, curY);
                doc.text('School Principal', sigRightX, curY);

                drawFooter(2, 3);

                // ════════════════════════════════════════════════════
                // ── PAGE 3: Detection Log ──
                // ════════════════════════════════════════════════════
                const detectionLog: Array<{ studentName: string; date: string; time: string; status: string; confidence: number | null }> = sf2.detectionLog || [];
                ...
                doc.setFont('helvetica', 'bold');
                doc.setTextColor(255, 255, 255);
                doc.text(`Detection Log — ${sf2.courseName} (Section ${sf2.section})`, pageW / 2, 9, { align: 'center' });
                curY = 18;

                doc.setFontSize(8);
                doc.setFont('helvetica', 'italic');
                doc.setTextColor(120, 120, 120);
                doc.text(`${sf2.month} ${sf2.year} — Best attendance record per student per day as detected by SAMS`, mL, curY);
                curY += 5;

                const logHead = [['No.', 'Student Name', 'Date', 'Time', 'Status', 'Confidence']];
                const logBody = detectionLog.map((det, i) => {
                    const statusColor: [number, number, number] = det.status === 'Present' ? green : det.status === 'Late' ? yellow : red;
                    const confPct = det.confidence != null ? `${(det.confidence * 100).toFixed(1)}%` : '-';
                    return [
                        { content: String(i + 1), styles: { halign: 'center' as const, textColor: gray } },
                        det.studentName,
                        { content: det.date, styles: { halign: 'center' as const } },
                        { content: det.time, styles: { halign: 'center' as const } },
                        { content: det.status, styles: { halign: 'center' as const, fontStyle: 'bold' as const, textColor: statusColor } },
                        { content: confPct, styles: { halign: 'center' as const } },
                    ];
                });
                ...
                autoTable(doc, {
                    startY: curY,
                    head: logHead,
                    body: logBody.length > 0 ? logBody as any : [[{ content: 'No detections found for this period.', colSpan: 6, styles: { halign: 'center' as const, fontStyle: 'italic' as const, textColor: gray } }]],
                    theme: 'grid',
                    styles: { fontSize: 8.5, cellPadding: 2.5, lineColor: [180, 180, 180], lineWidth: 0.15 },
                    headStyles: { fillColor: navy, textColor: [255, 255, 255], fontStyle: 'bold', halign: 'center', cellPadding: 3 },
                    columnStyles: {
                ...
                // Summary strip
                doc.setFillColor(...lightBg);
                doc.roundedRect(mL, curY, contentW, 9, 1.5, 1.5, 'F');
                doc.setFontSize(8);
                doc.setFont('helvetica', 'bold');
                doc.setTextColor(...navy);
                doc.text(`Total Records: ${detectionLog.length}`, mL + 4, curY + 5.5);
                doc.setFont('helvetica', 'italic');
                doc.setFontSize(7);
                doc.setTextColor(140, 140, 140);
                doc.text('This log contains the best attendance record per student per day as detected by the SAMS camera module.', mL + 50, curY + 5.5);
                ...
                doc.save(`SF2_${sanitizedCourse}_Sec${sanitizedSection}_${sf2.month}_${sf2.year}.pdf`);

            } else {
            // ═══════════════════════════════════════════════════
            // ── Excel Export ──
            // ═══════════════════════════════════════════════════
            const totalCols = 2 + allDays.length + 5; // No. + Name + days + 5 summary cols (P, L, A, ND, %)
            ...
            for (let i = 0; i < students.length; i++) {
                ...
                for (let d = 0; d < allDays.length; d++) {
                    const status = student.dailyStatus[String(allDays[d])];
                    if (status === 'P') {
            ...
            let R2 = 0;
            const setCell2 = (r: number, c: number, value: any, style?: any) => {
                const cellRef = XLSX.utils.encode_cell({ r, c });
                const cell: any = { v: value, t: typeof value === 'number' ? 'n' : 's' };
                if (style) cell.s = style;
                ws2[cellRef] = cell;
            ...
            // Title row
            setCell2(R2, 0, `Detection Log — ${sf2.courseName} (Section ${sf2.section})`, titleStyle);
            for (let c = 1; c < 6; c++) setCell2(R2, c, '', titleStyle);
            merges2.push({ s: { r: R2, c: 0 }, e: { r: R2, c: 5 } });
            R2++;

            // Subtitle
            setCell2(R2, 0, `${sf2.month} ${sf2.year} — All face recognition detections recorded by SAMS`, {
            ...
            const logHeaders = ['No.', 'Student Name', 'Date', 'Time', 'Status', 'Confidence'];
            logHeaders.forEach((h, c) => setCell2(R2, c, h, logHeaderStyle));
            R2++;
            ...
            if (detectionLog.length === 0) {
                ...
                for (let i = 0; i < detectionLog.length; i++) {
                    const det = detectionLog[i];
                    const isEven = i % 2 === 0;
                    const rowBg = isEven ? 'FFFFFF' : 'F8FAFB';

                    const statusColor = det.status === 'Present' ? '0F9D58' : det.status === 'Late' ? 'F4B400' : 'DB4437';
                    const confPct = det.confidence != null ? `${(det.confidence * 100).toFixed(1)}%` : '-';

                    setCell2(R2, 0, i + 1, { alignment: { horizontal: 'center' }, border, fill: { fgColor: { rgb: rowBg } }, font: { sz: 10, color: { rgb: '828282' } } });
                    setCell2(R2, 1, det.studentName, { border, fill: { fgColor: { rgb: rowBg } }, font: { sz: 10 } });
                    setCell2(R2, 2, det.date, { alignment: { horizontal: 'center' }, border, fill: { fgColor: { rgb: rowBg } }, font: { sz: 10 } });
                    setCell2(R2, 3, det.time, { alignment: { horizontal: 'center' }, border, fill: { fgColor: { rgb: rowBg } }, font: { sz: 10 } });
                    setCell2(R2, 4, det.status, { alignment: { horizontal: 'center' }, border, fill: { fgColor: { rgb: rowBg } }, font: { bold: true, sz: 10, color: { rgb: statusColor } } });
                    setCell2(R2, 5, confPct, { alignment: { horizontal: 'center' }, border, fill: { fgColor: { rgb: rowBg } }, font: { sz: 10 } });
                    R2++;
                }
            }
            ...
            // Summary
            R2++;
            setCell2(R2, 0, `Total Records: ${detectionLog.length}`, { font: { bold: true, sz: 10, color: { rgb: '333333' } } });
            R2++;
            setCell2(R2, 0, 'This log contains the best attendance record per student per day as detected by the SAMS camera module.', { font: { italic: true, sz: 9, color: { rgb: 'AAAAAA' } } });
            R2++;

            // Set range, merges, col widths
            ...
            // Row heights
            const rowHeights2: XLSX.RowInfo[] = [];
            rowHeights2[0] = { hpx: 28 };
                                        ...
                                        <div className="teacher-filters">
                                            ...
                                            <div className="teacher-export-wrapper" ref={exportPickerRef}>
                                                <button
                                                    onClick={() => {
                                                        if (!selectedOverviewCourse || !selectedSection) {
                                                            alert('Please select a course and section first to export the attendance report.');
                                                            return;
                                                        }
                                                ...
                                                {showExportPicker && (
                                                    <div className="teacher-export-popover">
                                                        <p className="teacher-export-popover-title">Export Report</p>
