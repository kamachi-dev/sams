...
export async function GET() {
    try {
        ...
        // Each course card represents one parent course (c.id) with all its sections.
        // We return a representative section ID (MIN s.id) so the frontend can call
        // the sections API which expects any section ID belonging to that course.
        ...
        const coursesWithSections = result.rows.map(row => {
            ...
            return {
                ...
                // Only expose a single schedule string when all sections share the same schedule;
                // otherwise leave it empty so the UI doesn't show a misleading value.
                schedule: sections.length === 1
                    ? (sections[0].schedule || '')
                    : (sections.every(s => s.schedule === sections[0].schedule) ? (sections[0].schedule || '') : ''),
